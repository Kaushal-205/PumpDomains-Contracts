...
contract DomainRecords {
    ...
    function recordDomain(
        ...
        emit DomainRecorded(nameWithTld, owner, registrationDate, expirationDate, registrationPrice, factoryAddress);
    }

    // Function to get all registered domains
    function getAllRegisteredDomains() external view returns (RegisteredDomain[] memory) {
        return registeredDomains;
    }

    // Function to get a specific domain by index
    function getDomainByIndex(uint256 index) external view returns (RegisteredDomain memory) {
    ...
    function getDomainsByFactory(address factoryAddress) external view returns (RegisteredDomain[] memory) {
        uint256[] memory indices = domainsByFactory[factoryAddress];
        RegisteredDomain[] memory domainsForFactory = new RegisteredDomain[](indices.length);
    ...
    function getDomainsByOwner(address owner) external view returns (RegisteredDomain[] memory) {
        uint256[] memory indices = domainsByOwner[owner];
        RegisteredDomain[] memory domainsForOwner = new RegisteredDomain[](indices.length);
